#include <sys/types.h>
#include <fcntl.h>
#include <dirent.h>
#include <errno.h>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>

/**
   A buffer that can be dynamically resized
   by other functions
*/
typedef struct {
	char* buf;
	ssize_t buf_size;
} dyn_buf;

/**
   A list of strings (similar to argv) that
   can be dynamically resized by other functions
*/
typedef struct {
	char** strings;
	ssize_t size;
} str_list;

/**
   Returns 1 if filename consists of digits (and
   therefore may represent a PID), 0 otherwise
*/
int is_pid(const char* filename);

/**
   Reads file and stores contents into buf field of buf.
   If there is not enough space, the function will resize
   the buffer until it fits all file contents.
   The filename is a relative path to a file, and dirfd is
   a directory descriptor
*/
ssize_t areadat(int dirfd, const char* filename, dyn_buf* buf);

/**
   Reads symlink and stores contents into buf.
   If there is not enough space, the function will resize
   the buffer until it fits all file contents.
   The filename is a relative path to a symlink file, and dirfd is
   a directory descriptor
*/
ssize_t areadlinkat(int dirfd, const char* path, dyn_buf* buf);

/**
   Glues together "/proc/", "[pid]" and "/[filename]" strings
   If file is NULL, the resulting string does not include filename,
   only "/proc/[pid]"
*/
const char* construct_full_path(const char* pid_str, const char* file);

/**
   Finds separate null-terminated strings in a text buffer and creates
   a list of strings similar to argv
*/
ssize_t tokenize(char* buf, ssize_t buf_size, str_list* list);
