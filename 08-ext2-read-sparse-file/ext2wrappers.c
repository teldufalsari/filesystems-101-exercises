...
#define BLOCK_O_ZEROS_SIZE 512
const char* g_block_o_zeros[BLOCK_O_ZEROS_SIZE] = {0};
...
{
	ssize_t actually_written = 0;
	while ((size_t)actually_written < size) {
		ssize_t current_write = write_exact(outfd, g_block_o_zeros, BLOCK_O_ZEROS_SIZE);
		if (current_write < 0) {
			return -1;
		} else {
			actually_written += current_write;
...
int copy_indirect(int imgfd, int outfd, unsigned size, unsigned block_size, unsigned* indir_block)
{
	...
	for (unsigned i = 0; i < block_ref_count; i++) {
		...
		if (indir_block[i] != 0) {
			...
			ret = write_zero_block(outfd, transfer_size);
		}
		if (ret < 0) {
			free(buf);
			return ret;
...
int copy_double_indirect(int imgfd, int outfd, unsigned size, unsigned block_size, unsigned* dindir_block)
{
	...
	for (unsigned i = 0; (i < block_ref_count) && (size_left > 0); i++) {
		if (dindir_block[i] == 0) { // We encountered an indirect block which is a hole
			unsigned max_size_ref_by_indir = block_ref_count * block_size;
			unsigned transfer_size = (size_left < max_size_ref_by_indir ? size_left : max_size_ref_by_indir);
			ret = write_zero_block(outfd, transfer_size);
			if (ret < 0) {
		...
		} else { // we encountered a regular indirect block
...
int copy_by_inode(int imgfd, int outfd, unsigned block_size, const struct ext2_inode* inode_ptr)
{
	...
	if (size_left > 0) {
		if (inode_ptr->i_block[EXT2_DIND_BLOCK] == 0) { // File has a "tail" of zeros
			ret = write_zero_block(outfd, size_left);
		} else {
